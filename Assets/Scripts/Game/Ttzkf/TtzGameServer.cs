...
{
    public class TtzGameServer : YxGameServer
    {
        ...
        private void OnUserReady(ISFSObject responseData)
        {
            ...
            App.GetGameManager<TtzGameManager>().OnUserReady(serverSeat);
        }
        ...
        public void OnHandsUp(ISFSObject requestData)
        {
            ...
            int time = App.GetGameData<TtzGameData>().HupTime;
            if (requestData.ContainsKey(InteractParameter.CdTime))
            {
                time = requestData.GetInt(InteractParameter.CdTime);
            }
            App.GetGameManager<TtzGameManager>().HupWindow.ShowHandUp(data, time);
        }
