    ...
    public class MenuCtrl : MonoBehaviour
    {
        ...
        public void OnClickWeiChatInviteBtn()
        {
            ...
            YxFramwork.Controller.UserController.Instance.GetShareInfo(info =>
            {
                ...
                string.Format(
                    "[扑克];房间号:[{0}];{1}",
                    App.GetGameData<NnGameData>().RoomType
                    ,
                    App.GetGameData<NnGameData>().Rule
                );
                ...
                (window, btnname) =>
                {
                    switch (btnname)
                    {
                        case YxMessageBox.BtnLeft:
                            App.GetRServer<NnGameServer>().StartHandsUp(2);             // -1拒绝 2发起投票 3同意                            
                            break;
                    }
                },
                true,
        ...
        public void Zhanji()
        {
            App.GetGameManager<NnGameManager>().Result.ShowBg(true);
        }
    }
}
