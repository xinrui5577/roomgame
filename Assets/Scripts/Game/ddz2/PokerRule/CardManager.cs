...
/*            foreach (var cds in dic.Keys)
        ...
        private List<int[]> CheckTishiGroup(int[] hdCds, int[] lastoutCds, CardType getlastOutCdsType = CardType.None)
        {
            ...
            switch (lastOyutType)
            {
                ...
                case CardType.C1112223344:
                    CheckTishiGpWhenLstOutC1112223344(ref cdsTypeDic, cdSplit, lastoutCds);
                    break;
       ...
       /*         case CardType.C422:
            ...
            var superBomb = new List<int[]>();
            var tianzha = new List<int[]>();
            foreach (var cds in cdsTypeDic.Keys)
            {
                switch (cdsTypeDic[cds])
                {
                    case CardType.C4:
                        zhadan.Add(cds);
                        continue;
            ...
            tishiList.AddRange(superBomb);
            tishiList.AddRange(tianzha);
            //-------------------------------------------------------------------------end
        ...
        private void CheckTishiGpWhenLstOutC1112223344(ref Dictionary<int[], CardType> cdsTypeDic, CdSplitStruct cdSplit,
                                       int[] lastoutCds)
        {
            var minlastotCdvalue = GetMinValue3PartOfCds(lastoutCds);
            if (minlastotCdvalue == -1) return;

            CheckC1112223344(ref cdsTypeDic, cdSplit);
            var intsNeedRemove = new List<int[]>();
            foreach (var cds in cdsTypeDic.Keys)
            {
                if (cdsTypeDic[cds] != CardType.C1112223344 ) continue;
...
*/
        ...
        private void CheckC1112223344(ref Dictionary<int[], CardType> cdsTypeDic, CdSplitStruct cdSplit)
        {
            var listTemp = new List<int>();
            listTemp.AddRange(cdSplit.ThreeCds);
            listTemp.AddRange(cdSplit.FourCds);
            ...
            foreach (var cds in allshunList)
            {
                ...
                if (duiCdsList.Count >= sanLen)
                {
                    ...
                    cdsTypeDic[c1112223344Cds.ToArray()] = CardType.C1112223344;
                }
            }
        }
...
/*
        ...
        /// <summary>
        /// 检测某一组连3牌可以组成的C11122234 ，C1112223344
        /// </summary>
    ...
    internal class CheckAllposbCdsType
    {
        ...
        public Dictionary<int[], CardType> GetCdsTypeDic()
        {
            ...
            switch (totalNum)
            {
                ...
                case 3:
                    return new GetCdsTypeToDicCdsLen3(CdsWithOutLzList, LaiZiNum).GetCdsTypeDic();
                case 4:
            ...
            keyList.AddRange(new CardType[]{CardType.C1, CardType.C111222, CardType.C1112223344, 
                CardType.C11122234, CardType.C1122, CardType.C123, CardType.C2,CardType.C3, 
                CardType.C31, CardType.C32, CardType.C4, CardType.C411, CardType.C42, CardType.C5, });
