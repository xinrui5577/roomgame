...
public class tk2dUIItem : MonoBehaviour
{
    ...
    /// </summary>
    public event System.Action<GameObject> OnHoverOutUIItem;
    ...
    private void DoSendMessage( string methodName ) {
        if (sendMessageTarget != null && methodName.Length > 0) {
            sendMessageTarget.SendMessage( methodName, gameObject, SendMessageOptions.RequireReceiver );
        }
    }
    ...
    public void HoverOut(tk2dUIItem currHoverButton)
    {
        if (isHoverOver)
        {
            ...
            if (OnHoverOutUIItem != null) { OnHoverOutUIItem(gameObject); }
            isHoverOver = false;
        }
