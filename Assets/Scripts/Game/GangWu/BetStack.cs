...
namespace Assets.Scripts.Game.GangWu
{
    public class BetStack : MonoBehaviour
    {
        ...
        public void SendBetToSomewhere(List<Transform> somewhere)
        {
            ...
            //生成筹码 最多16个
            GameObject[] betObjs =
                App.GetGameManager<GangWuGameManager>()
                    .BetMgr.CreatBetArray(
                        int.TryParse(Bet.text, out allBet) ? allBet : App.GetGameData<GangwuGameData>().Ante*500, 16,
                        transform);
            ...
            for (int i = 0; i < Bets.Count; i++)
            {
                ...
                bet.BeginMove(bet.transform.localPosition, Vector3.zero, i * App.GetGameData<GangwuGameData>().BetSpeace, BetFinishedType.Destroy,
                    () =>
                    {
