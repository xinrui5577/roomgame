...
namespace Assets.Scripts.Game.sssjp
{
    public class PokerCard : MonoBehaviour
    {
        ...
        public void SetCardFront()
        {
            ...
            var numTran = Num.GetComponent<PokerUnitScale>();
            if (numTran != null)
            {
                numTran.SetScalue();
            }
            var SmallColorTran = SmallColor.GetComponent<PokerUnitScale>();
            if (SmallColorTran != null)
            {
                SmallColorTran.SetScalue();
            }
            var BigColorTran = BigColor.GetComponent<PokerUnitScale>();
            if (BigColorTran != null)
            {
                BigColorTran.SetScalue();
            }
        }

        private void SetSprite()
        {
            var spriteName = GetCardValueStr();
            Sprite.spriteName = spriteName;
            var btn = Sprite.GetComponent<UIButton>();
            if (btn != null)
        ...
        public void PokerJump()
        {
            ...
            {
                Debug.LogError(string.Format("牌{0}不能执行跳跃动画，其active为false", name));
            }
        }
